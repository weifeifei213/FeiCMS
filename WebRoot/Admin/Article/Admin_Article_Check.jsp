...
List<SiteClass> sclist=scd.queryTopClassList();

String inputer = (String) session.getAttribute("AdminName");

String strSql="";
if(inputer!=null&&!"".equals(inputer)&&(inputer.equals("admin")||inputer.equals("办公室")||inputer.equals("bgs"))){
strSql="select * from  article  where Deleted=0 ";

}else{
 strSql = "select * from  article  where Deleted=0 and Inputer='"
			+ inputer + "'";
...
List<Article> artlist=DAOFactory.getArticleDAOInstance().executeQuery(strSql);
     ...
     function throwGO(){   
		...
		var articleID= document.getElementById("ArticleID").value;
			var getNum=document.getElementById("getNum").value;
				form.action="Admin_Article_ModfySave.jsp?Action=SetPassed&ArticleID="+articleID+"&getNum="+getNum;
				form.submit();
		...
		<table width='100%' border='0' align='center' cellpadding='2'
			...
			<tr class='tdbg'>
				...
				<td colspan='5'>
					...
					<a
						href='Admin_Article_Add.jsp'>添加文章</a>&nbsp;|&nbsp;
					<a href='Admin_Article_Check.jsp'>审核文章</a>&nbsp;|&nbsp;
					<a
						href='Admin_Article_Recyclebin.jsp'>文章回收站管理</a>
				</td>
			</tr>
			<form name='form3' method='Post' id="form2"
				action='Admin_Article_Check.jsp?Action=Search'>
			<tr class='tdbg'>
				<td width='70' height='30'>
					<strong>文章选项：</strong>
				</td>
		...
		<table width='100%' border='0' align='center' cellpadding='0'
			...
			<tr>
				<td height='22'>
					您现在的位置：&nbsp;文章中心管理&nbsp;&gt;&gt;&nbsp;
					<a
						href='Admin_Article_Check.jsp'>文章审核</a>&nbsp;&gt;&gt;&nbsp;所有文章！</td>
						...
						<tr class='tdbg' onMouseOut="this.className='tdbg'"
							...
							<td>
								[<font
									color='gray'>
									<%
									if(scd.getClassName(art.getClassID())==null){
									  out.println("不属于任何栏目"); 
									}else{
									%>
									<a href=Admin_Article_List.jsp?ClassID=<%=art.getClassID() %>>
									<%
									  out.println(scd.getClassName(art.getClassID())); 
									  }
									%></a></font>]&nbsp;
						  ...
						  <% 
							...
							<td width='238' align='center'>
								<a href='Admin_Article_ModfySave.jsp?Action=Reject&ArticleID=<%=art.getArticleID() %>'>直接退稿</a>&nbsp;&nbsp;
								<a href='Admin_Article_ModfySave.jsp?Action=SetPassed&ArticleID=<%=art.getArticleID() %>'>审核</a>&nbsp;&nbsp;
...
<input name='submit1' type='submit' value=' 审核通过 '
		...
		<div class="show_page">
			...
			<%
			if(pageCount>1){
			   out.println("<a href=Admin_Article_Check.jsp?page=1>首页</a> <a href=Admin_Article_Check.jsp?page="+(currPage-1)+">上一页</a>");
			   }
			else
			{
			    out.println("首页 上一页");
			...
			{
			   out.println("<a href=Admin_Article_Check.jsp?page="+(currPage+1)+">下一页</a> <a href=Admin_Article_Check.jsp?page="+pageCount+">尾页</a>");
			   }
			else
			{
			   out.println("下一页 尾页");
		...
		<form method='Post' name='SearchForm'
			action='Admin_Article_Check.jsp?Action=Search&Type=All'>
			<table width='100%' border='0' cellpadding='0' cellspacing='0'
				class='border'>
				<tr class='tdbg'>

...
cities['甘肃省']=new Array('兰州市', '嘉峪关市', '金昌市', '白银市', '天水市', '酒泉地区', '张掖地区', '武威地区', '庆阳市', '平凉地区', '定西地区', '陇南地区', '临夏回族自治州', '甘南藏族自治州');
...
Areas=new Object();
Areas['庆阳市']=new Array('西峰区','正宁县','宁县','环县','镇原县');
...
if(cv=='0') return;
if(typeof(Areas[cv])=='undefined') return;

  for(i=0; i<Areas[cv].length; i++)
  {
     ii = i+1;
     Area.options[ii] = new Option();
     Area.options[ii].text = Areas[cv][i];
     Area.options[ii].value = Areas[cv][i];
  }
}
function SetAddValue(){
var v_p=document.getElementById("H_Province");
var v_c=document.getElementById("H_City");
var v_area=document.getElementById("H_Area");
var v_add=document.getElementById("H_Address");
alert(v_p.value);

//选择省份
document.getElementById("province").value=v_p.value;
set_city(document.getElementById("province"), document.getElementById('city'));

document.getElementById("city").value=v_c.value;
set_Areas(document.getElementById("city"), document.getElementById('Area'));

document.getElementById("Area").value=v_area.value;
document.getElementById("addr").value=v_add.value;
						...
						<select name=province
							onchange="set_city(this, document.getElementById('city'));"  class="" >
							<option value="">
								全部省
							</option>
							<option value=北京市>
						...
						<select id=city name="city"  class=""  onchange="set_Areas(this, document.getElementById('Area'));" >
							<option value="">
								全部城市
							</option>
						</select>
						<select id=Area name="Area"  class="AddrSel"  >
							<option value="">
								请选择
							</option>
						</select>
						<input type="text" name="addr" class="" style="width: 100px">

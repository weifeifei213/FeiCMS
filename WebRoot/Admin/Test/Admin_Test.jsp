...
ResultSet rs = DAOFactory.getTestDaoInstance().showAllTest();
%>
<html>
	<head>
		<title>模拟管理</title>
 ...
 if(document.myform.Action.value=='Del'){
     if(confirm('确定要删除选中的调查吗？'))
		...
		<table width='100%' border='0' cellpadding='0' cellspacing='0'>
			<tr>
				<form name='myform' method='Post' action='Admin_Test_Modfy.jsp'
				...
				<td>
					<table class='border' border='0' cellspacing='1' width='100%'
						...
						<tr class='title'>
							...
							<td width="745" height='22' align='center'>
								<strong>试卷名称</strong>							</td>
							<td width='109' height='22' align='center'>
								<strong>试卷状态</strong>							</td>
							<td width='136' height='22' align='center'>
								<strong>发布时间</strong>							</td>
							<td width='192' height='22' align='center'>
								<strong>操作</strong>							</td>
						</tr>
						<%
						   int pageSize=20; 
						...
						<tr class='tdbg' onMouseOut="this.className='tdbg'"
							...
							<td>
								<a href='Admin_Test_Modfy.jsp?Action=Modfy&ID=<%=rs.getInt("ID") %>'><%=rs.getString("TestName")%>
								</a>
							</td>
							...
							<a
									href='Admin_Question_Add.jsp?ID=<%=rs.getInt("ID")%>'>添加试题</a>&nbsp;
								<a
									href='Admin_Test_Modfy.jsp?Action=Modfy&ID=<%=rs.getInt("ID")%>'>修改</a>&nbsp;
								<a
									href='Admin_Test_Modfy.jsp?Action=Del&ID=<%=rs.getInt("ID")%>'
					...
					<table width='100%' border='0' cellpadding='0' cellspacing='0'>
						<tr>
							...
							<td>
								<input type='submit' value='删除选定的试卷' name='submit'
			...
			<%
			if(pageCount>1){
			   out.println("<a href=Admin_Test.jsp?page=1>首页</a> <a href=Admin_Test.jsp?page="+(currPage-1)+">上一页</a>");
			   }
			else
			{
			    out.println("首页 上一页");
			...
			{
			   out.println("<a href=Admin_Test.jsp?page="+(currPage+1)+">下一页</a> <a href=Admin_Test.jsp?page="+pageCount+">尾页</a>");
			   }
			else
			{
			   out.println("下一页 尾页");
